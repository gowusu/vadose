#input function ############################################
#default function
#' @title R optmisation of Fredlund three-parameter particle size distribution function
#' @description The function optimises 3 parameters of Fredlund et al (2000) particle size distribution function
#' @author George Owusu
#' @inheritParams BEST
#' @param a an initial value of 'a' parameter to be optimised
#' @param m an initial value of 'm' parameter to be optimised
#' @param n an initial value of 'n' parameter to be optimised
#' @inheritParams lass3
#' @family Fredlund functions
#' @seealso \code{\link{zhuang3}}, \code{\link{zhuang4}},\code{\link{lass3}},\code{\link{logarithmic}},
#' \code{\link{logistic2}}, \code{\link{logistic3}},\code{\link{logistic4}},\code{\link{fredlund4}},
...
#' mod=fredlund3 (data=single,p="sand",D="D",fr="Sand.")
#' plot(mod)
#' 
#' #group simulation
#' mod2=fredlund3 (data=data,D="D",fr="FractionSand",group="ID")
#' mod2$coef
#' #generic function
#generic function
#' @rdname fredlund3
fredlund3<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,m=1,n=1) UseMethod ("fredlund3")
#' @export
#' @rdname fredlund3
#default function
fredlund3.default<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,m=1,n=1)
{
  ...
  if(is.null(data))
  {
    ...
    dmin=0.001
    fr=((1/(log(exp(1)+(a/D)^n))^m)*(1-(log(1+(1/D))/log(1+(0.01/dmin)))^7))
  ...
  if(!is.null(group)){
    ...
    addoutput=data.frame(groupid=factor(),a=numeric(),m=numeric(),n=numeric(),porosity=numeric(),r2=numeric(),
    ...
    while(i<=length(aggdata)){
      ...
      mod1=fredlund3(single,p=p2)
      gof1=gof(mod1)
      ######################################
      a=coef(mod1$fredlund3)[[1]]
      m=coef(mod1$fredlund3)[[2]]
      n=coef(mod1$fredlund3)[[3]]
      ...
      #######################################################
      addoutput=rbind(addoutput,data.frame(groupid=aggdata[i],a=a,m=m,n=n,porosity=mod1$p,r2=gof1$r2,
    ...
    if(is.null(p)){
      mod=fredlund3 (data=data,p="loam",D=D,fr=fr)
      p=texture(mod)
    }
    ...
    ##############################################################
    dmin=min(data[[D]])
    PDf=paste(fr,"~((1/(log(exp(1)+(a/",D,")^n))^m)*(1-(log(1+(1/",D,"))/log(1+(0.01/",dmin,")))^7))")
    ones=c(a=a,m=m,n=n)
    fredlund3<- nlxb(PDf, start = ones, trace = FALSE, data = data)
    #print(coef(fredlund3))
    ...
    #PD=((1/(log(exp(1)+(a/data[[D]])^n))^m)*(1-(log(1+(1/data[[D]]))/log(1+(0.01/dmin))))^7)
    PD=((1/(log(exp(1)+(a/data[[D]])^n))^m)*(1-(log(1+(1/data[[D]]))/log(1+(0.01/dmin)))^7))
    x=seq(min(data[[D]]),max(data[[D]]),0.01)
    y=((1/(log(exp(1)+(a/x)^n))^m)*(1-(log(1+(1/x))/log(1+(0.01/dmin)))^7))
    ...
    while (i<=max(data[[D]])){
      ...
      PD2=((1/(log(exp(1)+(a/i)^n))^m)*(1-(log(1+(1/i))/log(1+(0.01/dmin)))^7))
    ...
    #texture#########################################
    two=((1/(log(exp(1)+(a/2)^n))^m)*(1-(log(1+(1/2))/log(1+(0.01/dmin)))^7))*100
    
    silt=((1/(log(exp(1)+(a/0.05)^n))^m)*(1-(log(1+(1/0.05))/log(1+(0.01/dmin)))^7))*100
    
    clay=((1/(log(exp(1)+(a/0.002)^n))^m)*(1-(log(1+(1/0.002))/log(1+(0.01/dmin)))^7))*100
    
    sand=100-silt 
    silt=silt-clay
    clay=clay
    #####################################
    factor<-list(PSDF=length(coef(fredlund3)),x=x,y=y,fredlund3=fredlund3,predict=PD,D=data[[D]],a=a,m=m,n=n,dmin=dmin,fr=data[[fr]],p=p,sand=sand[[1]],silt=silt[[1]], clay=clay[[1]])
  }


factor$call<-match.call()

class(factor)<-"fredlund3"
factor
}
#' @export
#' @rdname fredlund3
#plot function
plot.fredlund3<-function(x,main="Particle Size Distribution Function",xlab="Particle Diameter",ylab="Fraction",...)
{
  object=x
mod1=object
...
#' @export
#' @rdname fredlund3
coef.fredlund3<-function(object,...)
{
x<-object$fredlund3
coef=(coef(x))
return(coef)
...
#' @rdname fredlund3
#####################################################################
predict.fredlund3<-function(object,D,...)
{
x<-object$fredlund3
...
n=coef(x)[[3]]
dmin=object$dmin
PD2=((1/(log(exp(1)+(a/D)^n))^m)*(1-(log(1+(1/D))/log(1+(0.01/dmin)))^7))
