#generic function
#' @title R optmisation of Nemes et al (1999) four-parameter particle size distribution model
#' @description The function optimises 4 parameters of Nemes et al (1999) particle size distribution function
#' @author George Owusu
#' @inheritParams BEST
#' @param a an initial value of 'a' parameter to be optimised
#' @param b an initial value of 'b' parameter to be optimised
#' @param e an initial value of 'e' parameter to be optimised
#' @param z an initial value of 'z' parameter to be optimised

#' @inheritParams lass3
#' @inheritParams andersson
...
#' @export
#' @references 
#' Nemes, A., Wosten, J. H. M., Lilly, A., & Voshaar, J. H. O. (1999). 
#' Evaluation of different procedures to interpolate particle-size distributions 
#' to achieve compatibility within soil databases. Geoderma., 90, 187-202. 
#' @examples
...
#' @rdname gompertz
#generic function
gompertz<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,b=1,e=1,z=1) UseMethod ("gompertz")
#' @export
#' @rdname gompertz
#default function
gompertz.default<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,b=1,e=1,z=1)
{
  ...
  if(!is.null(group)){
    ...
    while(i<=length(aggdata)){
      ...
      mod1=gompertz(single,p=p2)
      gof1=gof(mod1)
      ######################################
      a=coef(mod1$gompertz)[[1]]
      b=coef(mod1$gompertz)[[2]]
      e=coef(mod1$gompertz)[[3]]
      z=coef(mod1$gompertz)[[4]]
    ...
    if(is.null(p)){
      mod=gompertz (data=data,p="loam",D=D,fr=fr)
      p=texture(mod)
    }
    ...
    ##############################################################
    PDf=paste(fr,"~(a+e*exp(-exp(-b*(",D,"-z))))")
    
    ones=c(a=a,b=b,e=e,z=z)
    gompertz<- nlxb(PDf, start = ones, trace = FALSE, data = data)
    ...
    #texture#########################################
    two=(a+e*exp(-exp(-b*(2-z))))*100
    silt=(a+e*exp(-exp(-b*(0.05-z))))*100
    clay=(a+e*exp(-exp(-b*(0.002-z))))*100
    sand=100-silt 
    silt=silt-clay
    clay=clay
    factor<- list(PSDF=length(coef(gompertz)),x=x,y=y,gompertz=gompertz,predict=PD,D=data[[D]],a=a,b=b,e=e,z=z,fr=data[[fr]],p=p,sand=sand[[1]],silt=silt[[1]], clay=clay[[1]])
  }

factor$call<-match.call()

class(factor)<-"gompertz"
factor
}
#' @export
#' @rdname gompertz
#plot function
plot.gompertz<-function(x,main="Particle Size Distribution Function",xlab="Particle Diameter",ylab="Fraction",...)
{
  object=x
mod1=object
...
#' @export
#' @rdname gompertz
predict.gompertz<-function(object,D,...)
{
x<-object$gompertz
