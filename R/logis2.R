#' @title R optmisation of a two-parameter logistic particle size distribution function
#' @description The function optimises 2-parameter logistic  particle size distribution function
...
#' mod=logistic2 (data=single,p="sand",D="D",fr="Sand.")
#' plot(mod)
#' \dontrun{
#' #group simulation
#' mod2=logistic2 (data=data,D="D",fr="FractionSand",group="ID")
#' mod2$coef
#' }
#' @rdname logistic2
...
#generic function
logistic2<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,b=1) UseMethod ("logistic2")
#' @export
#' @rdname logistic2
#default function
logistic2.default<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,b=1)
{
  ...
  if(!is.null(group)){
    ...
    addoutput=data.frame(groupid=factor(),a=numeric(),b=numeric(),porosity=numeric(),r2=numeric(),
    ...
    while(i<=length(aggdata)){
      ...
      mod1=logistic2(single,p=p2)
      gof1=gof(mod1)
      ######################################
      a=coef(mod1$logistic2)[[1]]
      b=coef(mod1$logistic2)[[2]]
      ...
      #######################################################
      addoutput=rbind(addoutput,data.frame(groupid=aggdata[i],a=a,b=b,porosity=mod1$p,r2=gof1$r2,
    ...
    if(is.null(p)){
      mod=logistic2 (data=data,p="loam",D=D,fr=fr)
      p=texture(mod)
    }
    ...
    ##############################################################
    PDf=paste(fr,"~(1/(1+a*exp(-b*",D,")))")
    
    ones=c(a=a,b=b)
    logistic2<- nlxb(PDf, start = ones, trace = FALSE, data = data)
    
    a=coef(logistic2)[1]
    b=coef(logistic2)[2]
    
    
    PD=(1/(1+a*exp(-b*data[[D]])))
    ...
    #texture#########################################
    two=(1/(1+a*exp(-b*2)))*100
    silt=(1/(1+a*exp(-b*0.05)))*100
    clay=(1/(1+a*exp(-b*0.002)))*100
    sand=100-silt 
    silt=silt-clay
    clay=clay
    factor=list(PSDF=length(coef(logistic2)),x=x,y=y,logistic2=logistic2,predict=PD,D=data[[D]],a=a,b=b,fr=data[[fr]],p=p,sand=sand[[1]],silt=silt[[1]], clay=clay[[1]])
  }
factor$call<-match.call()

class(factor)<-"logistic2"
factor
}
#' @export
#' @rdname logistic2
#plot function
plot.logistic2<-function(x,main="Particle Size Distribution Function",xlab="Particle Diameter",ylab="Fraction",...)
{
  object=x
mod1=object
...
#' @export
#' @rdname logistic2
coef.logistic2<-function(object,...)
{
x<-object$logistic2
coef=(coef(x))
return(coef)
...
#' @export
#' @rdname logistic2
predict.logistic2<-function(object,D,...)
{
x<-object$logistic2
