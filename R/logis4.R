#' @title R optmisation of a four-parameter logistic particle size distribution function
#' @description The function optimises 4-parameter logistic  particle size distribution function
...
#' mod=logistic4 (data=single,p="sand",D="D",fr="Sand.")
#' plot(mod)
#' \dontrun{
#' #group simulation
#' mod2=logistic4 (data=data,D="D",fr="FractionSand",group="ID")
#' mod2$coef
#' }
#' @rdname logistic4
#generic function
logistic4<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,b=1,e=1,z=1) UseMethod ("logistic4")
#' @export
#' @rdname logistic4
#default function
logistic4.default<-function(data=NULL,D=NULL,fr=NULL,p=NULL,group=NULL,a=1,b=1,e=1,z=1)
{
  ...
  if(!is.null(group)){
    ...
    while(i<=length(aggdata)){
      ...
      mod1=logistic4(single,p=p2)
      gof1=gof(mod1)
      ######################################
      a=coef(mod1$logistic4)[[1]]
      b=coef(mod1$logistic4)[[2]]
      e=coef(mod1$logistic4)[[3]]
      z=coef(mod1$logistic4)[[4]]
    ...
    if(is.null(p)){
      mod=logistic4 (data=data,p="loam",D=D,fr=fr)
      p=texture(mod)
    }
    ...
    ##############################################################
    PDf=paste(fr,"~(b+(a-b)/(1+(",D,"/e)^z))")
    
    ones=c(a=a,b=b,e=e,z=z)
    logistic4<- nlxb(PDf, start = ones, trace = FALSE, data = data)
    ...
    #texture#########################################
    two=(b+(a-b)/(1+(2/e)^z))*100
    silt=(b+(a-b)/(1+(0.05/e)^z))*100
    clay=(b+(a-b)/(1+(0.002/e)^z))*100
    sand=100-silt 
    silt=silt-clay
    clay=clay
    factor=list(PSDF=length(coef(logistic4)),x=x,y=y,logistic4=logistic4,predict=PD,D=data[[D]],a=a,b=b,e=e,z=z,fr=data[[fr]],p=p,sand=sand[[1]],silt=silt[[1]], clay=clay[[1]])
  }
factor$call<-match.call()

class(factor)<-"logistic4"
factor
}
#' @export
#' @rdname logistic4
#plot function
plot.logistic4<-function(x,main="Particle Size Distribution Function",xlab="Particle Diameter",ylab="Fraction",...)
{
  object=x
mod1=object
...
#' @export
#' @rdname logistic4
coef.logistic4<-function(object,...)
{
x<-object$logistic4
coef=(coef(x))
return(coef)
...
#' @export
#' @rdname logistic4
predict.logistic4<-function(object,D,...)
{
x<-object$logistic4
